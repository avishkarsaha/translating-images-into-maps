    ...
    def forward(
        ...
        if mode == "recursive":  # training
            ...
            for i in range(qlen):
                ...
                shifted_1mp_choose = torch.cat(
                    [
                        key.new_ones(bs, self.n_heads_ma, 1, 1),
                        1 - p_choose[:, :, i : i + 1, :-1],
                    ],
                    dim=-1,
                )
                ...
                for j in range(klen):
                    q[:, :, i : i + 1, j + 1] = (
                        shifted_1mp_choose[:, :, i : i + 1, j].clone()
                        * q[:, :, i : i + 1, j].clone()
                        + aw_prev[:, :, :, j].clone()
                    )
                aw_prev = (
                    p_choose[:, :, i : i + 1] * q[:, :, i : i + 1, 1:]
                )  # `[B, H_ma, 1, klen]`
            ...
            if self.n_heads_ma == 1:
                ...
                alpha = p_choose_i * exclusive_cumprod(
                    1 - p_choose_i
                )  # `[B, H_ma, 1 (qlen), klen]`
            else:
                p_choose_i = (torch.sigmoid(e_ma) >= 0.5).float()[:, :, 0:1]
            ...
            if eps_wait > 0:
                for b in range(bs):
                    ...
                    for h in range(self.n_heads_ma):
                        ...
                        if (
                            alpha[b, h, 0].nonzero()[:, -1].min().item()
                            >= leftmost + eps_wait
                        ):
                            alpha[b, h, 0, :] = 0  # reset
        ...
        if self.chunk_energy is not None:
            ...
            if efficient_decoding and mode == "hard":
                alpha = torch.cat(
                    [
                        alpha.new_zeros(bs, alpha.size(1), 1, klen - alpha.size(3)),
                        alpha,
                    ],
                    dim=3,
                )
                if additional > 0:
                    alpha_masked = torch.cat(
                        [
                            alpha_masked.new_zeros(
                                bs, alpha_masked.size(1), 1, additional
                            ),
                            alpha_masked,
                        ],
                        dim=3,
            ...
            if efficient_decoding and mode == "hard":
                value = value[
                    :,
                    max(0, self.bd_offset + bd_leftmost - self.w + 1) : self.bd_offset
                    + bd_rightmost
                    + 1,
                ]
        ...
        if efficient_decoding and mode == "hard" and alpha.sum() > 0:
            self.bd_offset += (
                alpha[:, :, 0, self.bd_offset :].nonzero()[:, -1].min().item()
            )
        ...
        for h in range(n_heads_mono):
            if alpha[b, h, 0, 0].sum() > 0:
                ...
                if chunk_size == -1:
                    ...
                    mask[
                        b, h, :, 0, max(0, boundary - chunk_size + 1) : boundary + 1
                    ] = 1
...
class CausalConv1d(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, dilation=1, param_init=""
    ):
